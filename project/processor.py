...
Functions are written in a procedural style for simplicity. This module
also exposes helper routines used by the Flask web application in
`app.py`.
"""
...
# web-related imports are placed below (Flask application initialization)
...
def get_top5_students(csv_file=DEFAULT_FILES["first_year_csv"]):
    """
...
def get_top5_students_second_year(csv_file=DEFAULT_FILES["second_year_csv"]):
    """
    Read first-year CSV (including fail students), sort students by CGPA, 
    and return top 5 students as a list of strings.
    Each string: "1. Name | PRN | CGPA: X.XX"
...
def get_top5_students_sgpa_wise_sem1(csv_file=DEFAULT_FILES["first_year_csv"]):
    """
...
def get_top5_students_sem1_second_year(csv_file=DEFAULT_FILES["second_year_csv"]) :
    """
    Read first-year CSV (including fail students), sort by SGPA 1,
    and return top 5 students.
    """
...
def get_top5_students_sem2_second_year(csv_file=DEFAULT_FILES["second_year_csv"]):
    """
...
def get_top5_students_sem3_second_year(csv_file=DEFAULT_FILES["second_year_csv"]):
    """
...
def get_top5_students_sem4_second_year(csv_file=DEFAULT_FILES["second_year_csv"]):
    """
...
def get_top5_students_sgpa_wise_sem2(csv_file=DEFAULT_FILES["first_year_csv"]):
    """
    Read first-year CSV (including fail students), sort by SGPA 2,
    and return top 5 students.
    """
...
def get_subject_toppers(csv_file=DEFAULT_FILES["first_year_csv"]):
    """
...
def get_subject_summary(csv_file=DEFAULT_FILES["first_year_all_csv"]):
    """
...
def get_subject_summary_2(csv_file=DEFAULT_FILES["second_year_csv"]):
    """
    Reads student data from the CSV and calculates:
    - Pass count
    - Fail count
...
def get_subject_toppers_2(csv_file=DEFAULT_FILES["second_year_csv"]):
    """
    Read first-year CSV (including fail students) and find toppers 
    for each subject (_TOTAL columns).
    Returns a list of strings for display:
...
# duplicate Flask import block removed; app already declared above
import matplotlib.pyplot as plt
import os
import matplotlib
matplotlib.use('Agg')
...
# --------------------------------------------------------
def get_sgpa_chart(csv_file=DEFAULT_FILES["first_year_all_csv"]):
    categories = {
        "Distinction (>=8.5)": 0,
        "O (8.0 - 8.49)": 0,
        "A+ (7.5 - 7.99)": 0,
...
# the Flask application and imports were defined earlier; skip them

# --------------------------------------------------------
# Function: Generate SGPA Distribution Chart
# --------------------------------------------------------
def get_sgpa_chart_sem2(csv_file=DEFAULT_FILES["first_year_all_csv"]):
    categories = {
        "Distinction (>=8.5)": 0,
        "O (8.0 - 8.49)": 0,
        "A+ (7.5 - 7.99)": 0,
...
def get_sgpa_chart_sem1_y2(csv_file=DEFAULT_FILES["second_year_csv"]):
    categories = {
        "Distinction (>=8.5)": 0,
        "O (8.0 - 8.49)": 0,
        "A+ (7.5 - 7.99)": 0,
...
def get_sgpa_chart_sem2_y2(csv_file=DEFAULT_FILES["second_year_csv"]):
    categories = {
        "Distinction (>=8.5)": 0,
        "O (8.0 - 8.49)": 0,
        "A+ (7.5 - 7.99)": 0,
...
def get_sgpa_chart_sem3_y2(csv_file=DEFAULT_FILES["second_year_csv"]):
    categories = {
        "Distinction (>=8.5)": 0,
        "O (8.0 - 8.49)": 0,
        "A+ (7.5 - 7.99)": 0,
...
def get_sgpa_chart_sem4_y2(csv_file=DEFAULT_FILES["second_year_csv"]):
    categories = {
        "Distinction (>=8.5)": 0,
        "O (8.0 - 8.49)": 0,
        "A+ (7.5 - 7.99)": 0,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Utility to run various processing steps outside of Flask."
    )
    parser.add_argument("--clean", "-c", help="Path to PDF file to clean")
    parser.add_argument("--first-csv", action="store_true",
                        help="Generate first-year CSVs from the default text file")
    parser.add_argument("--second-csv", action="store_true",
                        help="Generate second-year CSV from the default text file")
    args = parser.parse_args()

    if args.clean:
        result = clean_pdf(args.clean)
        print(result or "Clean complete")
    if args.first_csv:
        print(generate_first_year_csv())
        print(generate_all_first_year_csv())
    if args.second_csv:
        print(generate_second_year_csv())
